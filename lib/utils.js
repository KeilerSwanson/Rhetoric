...
function formatDate(dateStr) {
	if (typeof dateStr === 'string') {
		console.log('dateStr is string')
		const newDateStr = new Date(`${dateStr}Z`).toDateString().substring(4)
		const datePieces = newDateStr.split(' ')
		datePieces[1] += ','
		return datePieces.join(' ')
	}
	return 'Date Unavailable'
	// return (typeof dateStr === 'string') ? new Date(`${dateStr}Z`).toDateString().substring(4) : 'Date Unknown'
}
